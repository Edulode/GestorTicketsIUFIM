    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-700">Modifica la información del usuario: {{ $usuario->nombre }} {{ $usuario->apellido_paterno }}</p>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="apellido_paterno" name="apellido_paterno" value="{{ old('apellido_paterno', $usuario->apellido_paterno) }}" required
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" id="apellido_materno" name="apellido_materno" value="{{ old('apellido_materno', $usuario->apellido_materno) }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $usuario->created_at ? $usuario->created_at->format('d/m/Y H:i') : 'No disponible' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Última Actualización</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $usuario->updated_at ? $usuario->updated_at->format('d/m/Y H:i') : 'No disponible' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Total de Tickets</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $usuario->tickets->count() }} tickets creados</p>
                    </div>
                </div>
            </div>
        </div>
